...
import { Gift, User, Search, Heart, ShoppingCart } from "lucide-react";
import { Input } from "@/components/ui/input";

const categories = [
    // ... kategorileriniz aynı kalacak ...
  {
    name: "Yeni",
...
export function Navbar() {
  ...
  // NİHAİ HESAPLAMA MANTIĞI
  useEffect(() => {
    if (openCategory && navRef.current) {
      const triggerElement = navRef.current.querySelector(`[data-category-name="${openCategory}"]`);
      const menuElement = navRef.current.querySelector(`[data-menu-name="${openCategory}"]`);
      ...
      if (triggerElement && menuElement) {
        ...
        // 1. İdeal "ortalanmış" pozisyonu hesapla
        let idealAbsoluteLeft = triggerRect.left + (triggerRect.width / 2) - (menuRect.width / 2);

        // 2. Sağdan taşmayı NAV BAR'a göre kontrol et
        if (idealAbsoluteLeft + menuRect.width > navRect.right) {
          // Taşarsa, menüyü NAV BAR'ın sağına yasla
          idealAbsoluteLeft = navRect.right - menuRect.width;
        }

        // 3. Soldan taşmayı NAV BAR'a göre kontrol et
        if (idealAbsoluteLeft < navRect.left) {
          // Taşarsa, menüyü NAV BAR'ın soluna yasla
          idealAbsoluteLeft = navRect.left;
        }

        // 4. Mutlak pozisyonu, `nav` elementine göreli hale getir
        const relativeLeft = idealAbsoluteLeft - navRect.left;

        setMenuPosition({ left: `${relativeLeft}px` });
      }
    }
  }, [openCategory]);
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      ...
      <div className="border-b">
        <div className="mx-auto max-w-7xl px-6 sm:px-6 lg:px-8 mt-2">
          <nav ref={navRef} className="relative z-10" aria-label="Global">
                  ...
                  {categories.map((category) => (
                    ...
                    >
                      ...
                      <div
                        ...
                        style={openCategory === category.name ? menuPosition : {}}
                        className={`${
                          openCategory === category.name ? 'block' : 'hidden'
                        } absolute z-10 mt-3 w-screen max-w-4xl transform px-4 sm:px-0`}
                      >
                            ...
                            <div className="">
                              {category.subcategories.map((subcategory) => (
                                <Link
                                  key={subcategory}
                                  href="#"
                                  className="-m-3 block rounded-md p-2 hover:bg-gray-50 transition ease-in-out duration-150"
                                >
                                  <p className="text-base text-gray-900">{subcategory}</p>
                                </Link>
                              ))}
                            </div>
