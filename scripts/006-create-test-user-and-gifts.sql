-- Create a test profile directly (bypassing auth.users for testing purposes)
...
DECLARE
    test_user_id UUID;
BEGIN
    -- Get the test user ID we just created
    SELECT id INTO test_user_id FROM profiles WHERE username = 'TestKullanici' LIMIT 1;
    ...
    ('Bluetooth Hoparlör', 'Su geçirmez, taşınabilir Bluetooth hoparlör. Plaj, piknik ve açık hava etkinlikleri için mükemmel.', '/placeholder.svg?height=300&width=300', '₺400-800', 'https://example.com/bluetooth-hoparlor', 'Doğum Günü', 'Müzik Sever', ARRAY['Teknoloji', 'Müzik'], test_user_id, 21, 21),

    ('Yoga Matı ve Aksesuar Seti', 'Yüksek kaliteli yoga matı, blok ve kayış içeren komple set. Yoga ve fitness tutkunları için ideal.', '/placeholder.svg?height=300&width=300', '₺200-400', 'https://example.com/yoga-seti', 'Kendine Hediye', 'Fitness Tutkunu', ARRAY['Sağlık & Wellness', 'Spor'], test_user_id, 16, 16),

    ('Kahve Değirmeni', 'Manuel kahve değirmeni, taze çekilmiş kahve için. Kahve ritüelini seven herkes için mükemmel.', '/placeholder.svg?height=300&width=300', '₺300-600', 'https://example.com/kahve-degirmeni', 'Ev Hediyesi', 'Kahve Sever', ARRAY['Yiyecek & İçecek', 'Ev'], test_user_id, 13, 13),

    ('Kişiselleştirilmiş Fotoğraf Albümü', 'El yapımı, kişiselleştirilmiş fotoğraf albümü. Özel anıları saklamak için romantik bir hediye.', '/placeholder.svg?height=300&width=300', '₺150-300', 'https://example.com/fotograf-albumu', 'Yıldönümü', 'Romantik Partner', ARRAY['Kişiselleştirilmiş', 'Anı'], test_user_id, 25, 25);
